...
# --- 配置区 (Paths) ---
BASE_PATH = r"g:\我的云端硬盘\AI+项目\GEO\助眠器GEO"
CONTENT_FILE = os.path.join(BASE_PATH, "GEO_Content_Draft_SleepBot.md")
...
async def publish_to_zhihu():
    async with async_playwright() as p:
        ...
        try:
            ...
            if not title_match:
                 title_match = re.search(r"\*\*拟定标题\*\*: (.*)", full_content)
            
            title = title_match.group(1).strip() if title_match else "VOLTZMANN 智能助眠技术解析"
            body = re.sub(r"^# .*\n", "", full_content)
